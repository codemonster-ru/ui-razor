...
final class CmAdminShellParityTest extends TestCase
{
    ...
    public function testMatchesCanonicalSignificantDom(string $casePath, string $htmlPath): void
    {
        ...
        $views = new RazorEngine(
            ...
            cachePath: sys_get_temp_dir() . '/codemonster-ui-admin-shell-' . bin2hex(random_bytes(6)),
        );
        $actual = (new CmAdminShell($views))->render(new ComponentRenderContext($case['props'], $slots))->value();
    ...
    {
        $cases = dirname(__DIR__, 4) . '/contracts/admin-shell/cases';
