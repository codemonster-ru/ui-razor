...
final class CmMenuBarParityTest extends TestCase
{
    ...
    public function testMatchesCanonicalSignificantDom(string $casePath, string $htmlPath): void
    {
        ...
        $views = new RazorEngine(
            ...
            cachePath: sys_get_temp_dir() . '/codemonster-ui-menu-bar-' . bin2hex(random_bytes(6)),
        );
        $slots = [];
        ...
        $actual = (new CmMenuBar($views))->render(new ComponentRenderContext($case['props'], $slots))->value();
    ...
    {
        $cases = dirname(__DIR__, 4) . '/contracts/menu-bar/cases';
