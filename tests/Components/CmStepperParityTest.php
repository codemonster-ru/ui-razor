...
final class CmStepperParityTest extends TestCase
{
    ...
    public function testMatchesCanonicalSignificantDom(string $casePath, string $htmlPath): void
    {
        ...
        $views = new RazorEngine(
            ...
            cachePath: sys_get_temp_dir() . '/codemonster-ui-stepper-' . bin2hex(random_bytes(6)),
        );
        $slots = [];
        ...
        $actual = (new CmStepper($views))->render(new ComponentRenderContext($case['props'], $slots))->value();
    ...
    {
        $cases = dirname(__DIR__, 4) . '/contracts/stepper/cases';
