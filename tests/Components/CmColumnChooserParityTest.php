...
final class CmColumnChooserParityTest extends TestCase
{
    ...
    public function testMatchesCanonicalSignificantDom(string $casePath, string $htmlPath): void
    {
        ...
        $views = new RazorEngine(
            ...
            cachePath: sys_get_temp_dir() . '/codemonster-ui-column-chooser-' . bin2hex(random_bytes(6)),
        );
        $slots = [];
        ...
        $actual = (new CmColumnChooser($views))->render(new ComponentRenderContext($case['props'], $slots))->value();
    ...
    {
        $cases = dirname(__DIR__, 4) . '/contracts/column-chooser/cases';
