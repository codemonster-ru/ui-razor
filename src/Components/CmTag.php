...
final class CmTag implements ComponentInterface
{
    ...
    public function render(ComponentRenderContext $context): RenderedHtml
    {
        ...
        $classes = (new ClassBuilder())
            ->add('cm-tag')
            ->addWhen($tone !== 'neutral', "cm-tag--{$tone}")
            ->add($this->className($attributes))
            ->value();

        return RenderedHtml::fromTrustedString(rtrim($this->views->render('components.tag', [
            'classes' => $classes,
